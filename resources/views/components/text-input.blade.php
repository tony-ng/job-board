<div class="relative">
    @if($type != 'textarea')
        ...
        @endif
        <input type="{{ $type }}" x-ref="{{ $name }}" name="{{ $name }}" value="{{ $value }}" placeholder="{{ $placeHolder }}" id="{{ $name }}" {{ $attributes->class(['w-full round-md border-0 ring-1 text-sm bg-white px-2.5 py-1.5 placeholder:text-slate-400 focus:ring-slate-600', 'pr-8' => $formRef, 'ring-slate-200' => !$errors->has($name), 'ring-red-300' => $errors->has($name)]) }} />
    @else
